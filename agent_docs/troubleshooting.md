...
Foreign keys are normally declared directly in the SQLModel classes and
enforced through Alembic migrations.

If a table must be loaded before its parent rows exist, keep the field
nullable for that bootstrap period and add the constraint in a migration once
...
In practice, "when ready" means the schema and load order are stable enough
that the database can enforce referential integrity without blocking the ETL
or initial backfill.
