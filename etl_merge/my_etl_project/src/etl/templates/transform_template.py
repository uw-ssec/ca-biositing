...
def transform(data_sources: Dict[str, pd.DataFrame]) -> Optional[pd.DataFrame]:
    ...
    """
    print("Transforming raw data from multiple sources...")

    # --- 1. Input Validation ---
    # Check if all required data sources are present
    for source_name in EXTRACT_SOURCES:
        if source_name not in data_sources:
            print(f"Error: Required data source '{source_name}' not found.")
            return None
    ...
    for source_name, columns in required_columns.items():
        ...
        for col in columns:
            if col not in df.columns:
                print(f"Error: Required column '{col}' not found in source '{source_name}'.")
                return None
    ...
    print(f"Successfully transformed data. Result has {len(transformed_df)} records.")
