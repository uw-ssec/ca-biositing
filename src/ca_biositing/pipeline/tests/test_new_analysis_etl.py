...
def test_icp_etl_full(
    ...
    mock_transform_logger.return_value = create_mock_logger()
    mock_load_logger.return_value = create_mock_logger()
    ...
    with prefect_test_harness():
        ...
        test_raw_df = pd.DataFrame({
            ...
            "method_id": ["method_1"],
            "resource": ["resource_1"],
            "prepared_sample": ["sample_1"],
            "dataset": ["dataset_1"]
        })
        mock_gsheet_to_df.return_value = test_raw_df

        # 2. Mock Transform (normalize_dataframes needs to return a list of DFs with expected columns)
