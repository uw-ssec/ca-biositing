...
def transform_proximate_record(
    ...
    logger.info("Transforming raw data for ProximateRecord table")
    ...
    try:
        ...
        if 'record_id' in record_df.columns:
            ...
            if record_df.duplicated(subset=['record_id']).any():
                ...
                logger.warning(f"ProximateRecord: Removing duplicate record_ids from transform output: {dupes}")
                record_df = record_df.drop_duplicates(subset=['record_id'], keep='first')
        else:
            logger.error("record_id missing from ProximateRecord transform")
            return pd.DataFrame()

        return record_df
    except Exception as e:
        logger.error(f"Error during ProximateRecord transform: {e}")
        return pd.DataFrame()
