...
def transform_xrd_record(
    ...
    if raw_df is None:
        logger.error("raw_df is None for XrdRecord transform")
        return pd.DataFrame()
    ...
    if raw_df.columns.duplicated().any():
        ...
        logger.warning(f"XrdRecord: Duplicate columns found and removed: {dupes}")
    ...
    coerced_df = coercion_mod.coerce_columns(
        cleaned_df,
        int_cols=['repl_no', 'scan_low_nm', 'scan_high_nm'],
        float_cols=[],
    ...
    rename_map = {
        ...
        'scan_low_nm': 'scan_low_nm',
        'scan_high_nm': 'scan_high_nm',
        'qc_result': 'qc_pass',
        'note': 'note',
    ...
    try:
        ...
        logger.error(f"Error during XrdRecord transform: {e}")
        return pd.DataFrame()
